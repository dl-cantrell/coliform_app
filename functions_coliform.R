...
# sdwis violations query BY COUNTY ------------------------------------------------------------------------------
get_vio_cnty <- function(cnty_vio) {
  vio_query_cnty <- "select distinct 
					TINWSYS.NAME as water_system_name,
					TINWSYS.NUMBER0 as water_system_no,
					TINLGENT.NAME as 'regulating_agency',
					TMNVIOL.EXTERNAL_SYS_NUM as 'violation_id',
				...
				--	AND TINWSYS.NUMBER0 = ?sys
					...
					Where TINWSYS.D_PRIN_CNTY_SVD_NM = ?cnty
					order by tmnviol.FED_PRD_BEGIN_DT"
  
  
  # Interpolate the SQL query with parameters
  int_vio_query <- sqlInterpolate(sdwis_tdt, vio_query_cnty, cnty = cnty_vio)
...
# sdwis enforcement actions query BY COUNTY ------------------------------------------------------------------------------

get_ea_cnty <- function(sys_ea_cnty){
  ea_query_cnty <- "with pws as (		--public water system info
					select Distinct
					TINWSYS.TINWSYS_IS_NUMBER as 'connector',
					TINLGENT.name as 'regulating_agency',
					TINWSYS.NUMBER0 as 'water_system_no',
...
final as (select
	...
	where final.[analyte_code] in ('8000', '3100', '3029','3013', '3000')
	and 	final.[county] = ?cnty
	order by final.[determination_date] "

# Interpolate the SQL query with parameters
#sys_ea_cnty = "LOS ANGELES"
int_ea_query_cnty <- sqlInterpolate(sdwis_tdt,  ea_query_cnty, cnty = sys_ea_cnty)

# Execute the query, with the filled in ?sys from the step above
ea_df_cnty <- dbGetQuery(sdwis_tdt, int_ea_query_cnty)  %>%
  mutate(year = substr(determination_date, 1, 4))
